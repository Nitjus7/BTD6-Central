...
async function getNewsData() {
  try {
    const newsData = await(await fetch("https://raw.githubusercontent.com/Nitjus7/BTD6-Central-Data/main/news.json")).json()
    newsCards = newsData["body"]
    return "yay"
  } catch(error) {
    alert("There was an error while loading the news.")
    console.log(error)
    return "oops"
  }
}
...
async function main() {
  if (await getNewsData() != "oops") generateNewsCards()
